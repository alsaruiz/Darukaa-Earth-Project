...
export default function MapView({
  ...
  const updateMapSourcesAndLayers = useCallback((map) => {
    ...
    if (source) {
      ...
      // 1. Crisp white outer casing halo for 100% contrast against any background
      map.addLayer({
  ...
  return (
    <div
      id="map-container"
      className="relative w-full overflow-hidden rounded-lg bg-[#FAF5F7] border border-[#DEC8D4] select-none shadow-xs group"
    ...
    >
      ...
      >
        {projectedSites.map((site) => (
          <g key={site.id}>
            ...
            <path d={site.d} fill="#E11D48" fillOpacity={0.22} />
            ...
            {site.points.map((pt, pIdx) => (
              <g key={pIdx}>
                <circle cx={pt.x} cy={pt.y} r={5} fill="#FFFFFF" stroke="#E11D48" strokeWidth={2} />
                <circle cx={pt.x} cy={pt.y} r={2} fill="#E11D48" />
              </g>
            ))}
          </g>
        ))}
        ...
        <div className="absolute top-3 left-3 z-10 flex items-center gap-1 bg-white/95 backdrop-blur-xs p-1 rounded-md border border-[#DEC8D4] shadow-xs">
          ...
          {Object.values(BASEMAP_STYLES).map((l) => (
            <button
              ...
              className={`text-[11px] px-2.5 py-0.5 rounded transition-colors cursor-pointer ${
                activeLayer === l.id
      ...
      <div className="absolute bottom-3 left-3 flex flex-wrap items-center gap-2 text-xs text-[#0F0C0E] pointer-events-none font-mono z-10">
        <div className="flex items-center gap-2 bg-[#FAF5F7]/95 backdrop-blur-xs px-2.5 py-1 rounded-md border border-[#DEC8D4] shadow-xs">
          ...
          {sitesList.length === 1 && (
            <>
              ...
              <span className="text-[#D65D80] font-semibold">{sitesList[0].areaHa} ha</span>
              <span className="text-[#332A30]">({sitesList[0].perimeterKm} km)</span>
            </>
          )}
          {sitesList.length > 1 && (
            <>
              <span className="text-[#E6D8DF]">|</span>
              <span className="text-[#D65D80] font-semibold">{totalAreaHa} ha total</span>
              <span className="text-[#332A30]">({sitesList.length} parcels)</span>
            </>
          )}
        </div>
      </div>
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-1.5 z-10">
          {onEditPolygon && (
            <button
              ...
              className="w-8 h-8 flex items-center justify-center bg-[#FAF5F7]/95 text-[#D65D80] rounded border border-[#DEC8D4] hover:bg-[#FDF2F6] hover:border-[#D65D80] transition-colors shadow-xs cursor-pointer"
            >
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center bg-[#FAF5F7]/95 text-[#0F0C0E] rounded border border-[#DEC8D4] hover:bg-[#FDF2F6] hover:text-[#D65D80] hover:border-[#D65D80] transition-colors shadow-xs cursor-pointer"
          >
