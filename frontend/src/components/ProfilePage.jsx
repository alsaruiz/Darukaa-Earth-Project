    ...
    <div id="profile-page" className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-[#0F0C0E] bg-[#FAF5F7]/80 hover:bg-[#FAF5F7] border border-[#DEC8D4] hover:border-[#D65D80] rounded-md transition-colors cursor-pointer shadow-xs"
        >
      ...
      <div className="p-1 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-[#0F0C0E]">{user?.name}</h1>
                <span className="px-2 py-0.5 rounded text-[11px] font-semibold border bg-[#FAF0F4] text-[#D65D80] border-[#DEC8D4]">
                  Admin
                </span>
              </div>
              <p className="text-xs text-[#332A30] font-medium flex items-center gap-1.5 mt-0.5">
                <Mail className="w-3.5 h-3.5 text-[#4D4048]" />
                {user?.email}
              </p>
              <p className="text-xs text-[#332A30] font-medium flex items-center gap-1.5 mt-0.5">
                <Building2 className="w-3.5 h-3.5 text-[#4D4048]" />
        ...
        {!isEditing && (
          <div className="mt-5 pt-4 border-t border-[#F0E2EA]">
            <p className="text-xs text-[#1F181D] font-medium leading-relaxed">
      ...
      {isEditing && (
        <form onSubmit={handleSave} className="rounded-xl border border-[#DEC8D4] p-6 space-y-4 animate-fade-in">
          <h2 className="text-sm font-bold text-[#0F0C0E]">Edit Profile Information</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-[#1F181D] mb-1">Full Name</label>
              <input
                type="text"
                value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                className="w-full px-3 py-2 text-xs bg-[#FAF5F7]/80 text-[#0F0C0E] border border-[#DEC8D4] rounded-md focus:outline-none focus:border-[#D65D80] transition-colors"
                required
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#1F181D] mb-1">Email Address</label>
              <input
                type="email"
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-[#1F181D] mb-1">Organization</label>
              <input
                type="text"
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-[#1F181D] mb-1">Bio / Role Description</label>
              <textarea
                rows={3}
                value={formData.bio}
                onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
                className="w-full px-3 py-2 text-xs bg-[#FAF5F7]/80 text-[#0F0C0E] border border-[#DEC8D4] rounded-md focus:outline-none focus:border-[#D65D80] transition-colors resize-none"
              />
            </div>
          </div>
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-[#F0E2EA]">
            <button
              ...
              className="px-3 py-1.5 text-xs font-medium text-[#332A30] hover:text-[#0F0C0E] cursor-pointer"
            >
              Cancel
            </button>
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 border-t border-[#DEC8D4]">
          <div className="flex items-center gap-2 text-[#D65D80] mb-2">
            <FolderKanban className="w-4 h-4" />
            <span className="text-xs font-bold text-[#0F0C0E]">Access Scope</span>
          </div>
          <p className="text-xs text-[#332A30] font-medium leading-relaxed">
        ...
        <div className="p-4 border-t border-[#DEC8D4]">
          <div className="flex items-center gap-2 text-[#D65D80] mb-2">
            ...
            <span className="text-xs font-bold text-[#0F0C0E]">Geodetic Reference</span>
          </div>
          <p className="text-xs text-[#332A30] font-medium leading-relaxed">
            Parcels and boundary nodes are calculated in <span className="font-mono text-[#0F0C0E] font-bold">WGS 84 (EPSG:4326)</span> latitude & longitude coordinates.
          </p>
        </div>
      </div>
    </div>
