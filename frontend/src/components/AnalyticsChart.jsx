...
export default function AnalyticsChart({ analytics = [], selectedMetric = 'NDVI', onSelectMetric }) {
  ...
  return (
    <div id="analytics-chart-card" className="space-y-4">
      {/* Header & Controls */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#EBD7E2]">
        <h2 className="text-sm font-semibold text-[#0F0C0E]">Analytics</h2>
          ...
          return (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded transition-all whitespace-nowrap cursor-pointer ${
                isActive
                  ? 'bg-[#FDF2F6] text-[#D65D80] font-semibold border border-[#D65D80]/50 shadow-xs'
                  : 'border border-[#DEC8D4] text-[#332A30] hover:bg-[#FAF5F7]/70 hover:text-[#0F0C0E]'
              }`}
            >
              {m}
            </button>
      ...
      <div className="grid grid-cols-4 gap-2 py-3.5 border-b border-[#EBD7E2] text-center">
        <div>
          <div className="text-[11px] text-[#332A30] font-medium">Latest</div>
          <div className="text-sm font-semibold text-[#0F0C0E] mt-0.5">{stats.latest}</div>
        </div>
        <div>
          <div className="text-[11px] text-[#332A30] font-medium">Average</div>
          <div className="text-sm font-semibold text-[#0F0C0E] mt-0.5">{stats.avg}</div>
        </div>
        <div>
          <div className="text-[11px] text-[#332A30] font-medium">Minimum</div>
          <div className="text-sm font-semibold text-[#0F0C0E] mt-0.5">{stats.min}</div>
        </div>
        <div>
          <div className="text-[11px] text-[#332A30] font-medium">Maximum</div>
          <div className="text-sm font-semibold text-[#0F0C0E] mt-0.5">{stats.max}</div>
        </div>
      </div>
      ...
      <div className="relative pt-4" style={{ height: '240px' }}>
        {filteredData.length === 0 ? (
          <div className="h-full flex items-center justify-center text-xs text-[#332A30]">
