  ...
  return (
    <div id="dashboard-container" className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold tracking-tight text-[#0F0C0E]">Dashboard</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
        ...
        <div className="md:col-span-5 space-y-4">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-[#EBD7E2]">
            <h2 className="text-sm font-semibold text-[#0F0C0E]">Projects</h2>
            <button
              id="add-project-btn"
              type="button"
                ...
                return (
                  <div
                    ...
                    className={`relative w-full text-left p-3.5 rounded-lg border transition-all duration-150 flex items-center justify-between group cursor-pointer ${
                      isSelected
                        ? 'bg-[#FDF2F6]/70 border-[#D65D80]'
                        : 'border-[#DEC8D4] hover:bg-[#FAF5F7]/50 hover:border-[#D65D80] hover:-translate-y-[0.5px]'
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`w-8 h-8 rounded flex items-center justify-center shrink-0 transition-colors ${
                          isSelected
                            ? 'bg-[#FAF0F4] text-[#D65D80] border border-[#E6D0DC]'
                            : 'bg-[#FAF5F7] text-[#332A30] group-hover:text-[#0F0C0E]'
                        }`}
                      >
                      ...
                      <div className="min-w-0">
                        <div
                          className={`text-sm font-medium truncate ${
                            isSelected ? 'text-[#0F0C0E] font-semibold' : 'text-[#0F0C0E]'
                          }`}
                        >
                          {proj.name}
                        </div>
                        <div className="text-[11px] text-[#332A30] font-medium mt-0.5">
                          ID: {proj.id}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="relative" ref={isMenuOpen ? menuRef : null}>
                      <button
                        ...
                        className={`p-1.5 rounded transition-colors cursor-pointer ${
                          isMenuOpen || isSelected
                            ? 'text-[#0F0C0E] hover:bg-[#FAF0F4]'
                            : 'text-[#4D4048] hover:text-[#0F0C0E] hover:bg-[#FAF0F4]'
                        }`}
                      >
                      ...
                      {isMenuOpen && (
                        <div
                          className="absolute right-0 top-8 z-30 w-32 bg-[#FAFBFD] rounded-md border border-[#F0E2EA] shadow-md py-1 text-xs"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <button
                            type="button"
                            onClick={(e) => handleOpenDetails(e, proj)}
                            className="w-full text-left px-3 py-1.5 text-[#0F0C0E] hover:bg-[#FAF5F7] flex items-center gap-2 transition-colors cursor-pointer"
                          >
                            <Info className="w-3.5 h-3.5 text-[#332A30]" />
                            Details
                          </button>
                          <button
                            type="button"
        ...
        <div className="md:col-span-7 space-y-4">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-[#EBD7E2]">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-semibold text-[#0F0C0E]">
                {selectedProject ? selectedProject.name : 'Sites'}
              </h2>
              {selectedProject && (
                <span className="text-xs text-[#332A30] font-semibold bg-[#FAF5F7] px-2 py-0.5 rounded border border-[#DEC8D4]">
              ...
              <div className="flex items-center border border-[#DEC8D4] rounded p-0.5 bg-[#FAF5F7]">
                <button
                  ...
                  className={`px-2.5 py-1 text-xs rounded transition-colors flex items-center gap-1 cursor-pointer ${
                    sitesViewMode === 'list'
                      ? 'bg-[#FAF0F4] text-[#D65D80] font-semibold border border-[#D65D80]/40 shadow-xs'
          ...
          {selectedProject && (
            <button
              ...
              className="w-full text-left p-3.5 rounded-lg border border-[#DEC8D4] hover:border-[#D65D80] hover:bg-[#FAF5F7]/50 text-[#332A30] hover:text-[#D65D80] transition-colors duration-150 flex items-center justify-between group cursor-pointer"
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded bg-[#FAF5F7] text-[#D65D80] flex items-center justify-center shrink-0 border border-[#DEC8D4] group-hover:border-[#D65D80] transition-colors">
                  <Plus className="w-4 h-4 text-[#D65D80]" />
                </div>
                <div className="text-sm font-medium text-[#0F0C0E] group-hover:text-[#D65D80] transition-colors">Add site</div>
              </div>
          ...
          {loadingSites ? (
            ...
            <div className="space-y-2 pt-1">
              <div className="flex items-center justify-between text-xs text-[#332A30]">
                <span>Project sites geospatial overview</span>
                <span className="text-[#D65D80] font-semibold text-[11px]">Click any site polygon to open analytics</span>
              </div>
              <MapView
                sites={sites}
                height={380}
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border overflow-hidden transition-colors duration-150 ${
                      isExpanded ? 'border-[#D65D80]' : 'border-[#DEC8D4] hover:border-[#D65D80]'
                    }`}
                  >
                    {/* Site Item Row Header */}
                    <div
                      className="p-3.5 flex items-center justify-between cursor-pointer hover:bg-[#FAF5F7]/50 transition-colors select-none"
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <span className="text-sm font-semibold text-[#0F0C0E] truncate">
                          {site.name}
                        </span>
                      </div>
                    ...
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        <motion.div
                          ...
                          className="overflow-hidden border-t border-[#EBD7E2]"
                          style={{ willChange: 'height, opacity', transform: 'translateZ(0)' }}
                        >
                          <div className="p-3.5 space-y-3">
                            <div className="flex items-center justify-between text-xs text-[#332A30] font-medium">
