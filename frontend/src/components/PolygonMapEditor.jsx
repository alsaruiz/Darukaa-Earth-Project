...
export default function PolygonMapEditor({
  ...
  const updatePolygonLayers = useCallback(
    (map, ptsToUse = null) => {
      ...
      if (source) {
        ...
        // 2. Crisp white outer casing halo for 100% boundary contrast
        map.addLayer({
      ...
      if (isFirst && !closed && currentPoints.length >= 3) {
        el.innerHTML = `
          <div class="w-7 h-7 -m-1.5 flex items-center justify-center">
            <div class="w-4 h-4 rounded-full bg-emerald-500 border-2 border-white shadow-md transition-transform hover:scale-125"></div>
          </div>
        `;
        el.onclick = (e) => {
          e.stopPropagation();
        ...
        el.innerHTML = `
          <div class="w-7 h-7 -m-1.5 flex items-center justify-center">
            <div class="w-3.5 h-3.5 rounded-full bg-white border-2 border-[#E11D48] shadow-md transition-transform hover:scale-125"></div>
          </div>
        `;
      }
      ...
      for (let i = 0; i < edgesCount; i++) {
        ...
        midEl.innerHTML = `
          <div class="w-6 h-6 -m-1 flex items-center justify-center">
            <div class="w-2.5 h-2.5 rounded-full bg-white border border-[#E11D48] shadow-xs hover:scale-150 transition-transform"></div>
          </div>
        `;
  ...
  return (
    <div
      className="relative w-full rounded-lg border border-[#DEC8D4] overflow-hidden select-none shadow-xs group bg-[#FAF5F7]"
    ...
    >
      ...
      >
        ...
        {/* Crisp High-Contrast White Outer Halo Casing */}
        {svgBoundaryPath && (
          <path
            d={svgBoundaryPath}
            stroke="#FFFFFF"
        ...
        {/* Bold Vivid Crimson Primary Boundary Stroke */}
        {svgBoundaryPath && (
          <path
            d={svgBoundaryPath}
            stroke="#E11D48"
      ...
      <div className="absolute top-2.5 inset-x-2.5 z-20 flex flex-wrap items-center justify-between gap-2 pointer-events-auto">
        ...
        <div className="flex items-center gap-1.5 bg-[#FAF5F7]/95 backdrop-blur-xs p-1 rounded-md border border-[#DEC8D4] shadow-md">
          <button
            type="button"
            onClick={() => {
              setIsDrawingMode(true);
            ...
            className={`text-xs px-2.5 py-1 rounded flex items-center gap-1.5 font-medium transition-colors cursor-pointer ${
              isDrawingMode
                ? 'bg-[#E11D48] text-white shadow-xs'
                : 'text-[#0F0C0E] hover:bg-[#FAF0F4] hover:text-[#E11D48]'
            }`}
          >
          ...
          <button
            ...
            className="text-xs px-2 py-1 rounded text-[#0F0C0E] hover:bg-[#FAF0F4] hover:text-[#E11D48] flex items-center gap-1.5 font-medium transition-colors cursor-pointer"
          >
            <Square className="w-3.5 h-3.5 text-[#E11D48]" />
            Drop box
          </button>

          <div className="h-4 w-px bg-[#DEC8D4] mx-0.5" />
          ...
          <div className="hidden sm:flex items-center gap-1">
            {PRESETS.map((p) => (
              <button
                ...
                className="text-[11px] px-2 py-0.5 rounded text-[#332A30] hover:text-[#E11D48] hover:bg-[#FAF0F4] transition-colors cursor-pointer font-medium"
              >
                {p.name}
              </button>
            ))}
        ...
        <div className="flex items-center gap-1.5 bg-[#FAF5F7]/95 backdrop-blur-xs p-1 rounded-md border border-[#DEC8D4] shadow-md">
          ...
          <button
            ...
            className="text-xs px-1.5 py-1 text-[#332A30] hover:text-[#0F0C0E] disabled:opacity-30 flex items-center gap-1 cursor-pointer font-medium rounded hover:bg-[#FAF0F4]"
          >
          ...
          <button
            ...
            className="text-xs px-1.5 py-1 text-[#332A30] hover:text-[#E03137] flex items-center gap-1 cursor-pointer font-medium rounded hover:bg-[#FAF0F4]"
          >
      ...
      <div className="absolute bottom-2.5 inset-x-2.5 z-20 flex items-center justify-between pointer-events-none">
        ...
        <div className="bg-[#FAF5F7]/95 backdrop-blur-xs px-3 py-1.5 rounded-md border border-[#DEC8D4] shadow-md text-xs text-[#0F0C0E] flex items-center gap-2.5 font-mono pointer-events-auto">
          <span className="flex items-center gap-1.5">
            <MapPin className="w-3.5 h-3.5 text-[#E11D48]" />
            <span>
              Area: <strong className="text-[#E11D48]">{area} ha</strong>
            </span>
          </span>
          <span className="text-[#DEC8D4]">•</span>
          <span>Perimeter: <strong>{perimeter} km</strong></span>
          <span className="text-[#DEC8D4]">•</span>
          <span>{isClosed ? points.length - 1 : points.length} vertices</span>
        </div>
        ...
        <div className="flex items-center gap-1.5 pointer-events-auto">
          <button
            ...
            className={`px-2.5 py-1.5 rounded-md text-xs font-mono font-medium flex items-center gap-1.5 border shadow-md transition-colors cursor-pointer ${
              showJsonDrawer
                ? 'bg-[#0F0C0E] text-white border-[#0F0C0E]'
                : 'bg-[#FAF5F7]/95 text-[#0F0C0E] border-[#DEC8D4] hover:bg-[#FAF0F4] hover:text-[#E11D48]'
            }`}
          >
          ...
          {points.length >= 3 && !isClosed && (
            <button
              ...
              className="bg-[#E11D48] hover:bg-[#C4153C] text-white px-3.5 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1.5 shadow-md cursor-pointer active:scale-95 transition-all"
            >
      ...
      <div className="absolute top-16 right-2.5 z-20 flex flex-col gap-1 pointer-events-auto">
        <button
          ...
          className="w-7 h-7 flex items-center justify-center bg-[#FAF5F7]/95 text-[#0F0C0E] rounded border border-[#DEC8D4] hover:bg-[#FAF0F4] hover:text-[#E11D48] transition-colors shadow-xs cursor-pointer"
        >
      ...
      {showJsonDrawer && (
        <div className="absolute bottom-12 right-2.5 z-30 w-80 max-w-[calc(100%-20px)] bg-[#FAF5F7]/95 backdrop-blur-md rounded-lg border border-[#DEC8D4] p-3 shadow-xl pointer-events-auto transition-all animate-in fade-in">
          <div className="flex items-center justify-between pb-2 mb-2 border-b border-[#F0E2EA]">
            <span className="text-xs font-semibold text-[#0F0C0E] flex items-center gap-1.5">
            ...
            <button
              ...
              className="text-[#332A30] hover:text-[#0F0C0E] cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          <textarea
            ...
            className={`w-full font-mono text-[11px] px-2.5 py-1.5 bg-[#FAF6F8] text-[#0F0C0E] rounded border outline-none transition-colors ${
              coordError
                ? 'border-[#E03137] focus:border-[#E03137]'
                : 'border-[#DEC8D4] focus:border-[#E11D48]'
            }`}
          />
          {coordError && (
            <p className="text-[11px] text-[#E03137] mt-1 font-medium">{coordError}</p>
          )}
          <p className="text-[10px] text-[#332A30] font-medium mt-1.5">
